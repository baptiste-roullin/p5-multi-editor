...
onMounted(() => {
	...
	async function truchet(canvas: HTMLCanvasElement, tileCanvas: HTMLCanvasElement, params: Params, mode: string) {
		...
		*/
		...
		function circle(x, y, r, style) {
			...
			tileContext.strokeStyle = "blue"
			tileContext.stroke()
			/*
						tileContext.lineWidth = 30
						tileContext.strokeStyle = "red"
						tileContext.stroke()

						tileContext.lineWidth = 15
						tileContext.strokeStyle = "yellow"
						tileContext.stroke()*/
		};
	...
	[...document.querySelectorAll('canvas')].forEach(async (el) => {
		const params = { width: 800, height: 800, tile_size: 200, background_phase: 1, background: '#fff', curves_per_tile: 3, seed: 1111 }
		//@ts-ignore
		window.truchet(
			el,
...
<template>
	<main><canvas class="truchet-rotate" data-args="">
		</canvas></main>
</template>
